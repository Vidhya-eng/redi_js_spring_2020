...
function dressListInput() {
    ...
    <p>Delivery Cost :${dressList[i].shippingCost.cost.deliveryCharge}</p>
        ...
        addToCartBtn.addEventListener('click', function() {
            showCart.push({
                newDress: dressList[i].name,
                newPrice: dressList[i].price
            });
        });
        addToCartBtn.innerText = "Add to Cart";
        addToCartBtn.classList.add("cartBtn")
        addToCartBtn.classList.add("btn-toolbar")
...
function addShoppingCartList() {
    ...
    for (let i = 0; i < showCart.length; i++) {
        ...
        newShowList.innerHTML = `<span>${showCart[i].newDress}</span><span>${showCart[i].newPrice}</span>`;
...
function showTotalPrice() {
    let totalPrice = 0;
    for (let i = 0; i < showCart.length; i++) {
        totalPrice += parseFloat(showCart[i].newPrice);
    }
